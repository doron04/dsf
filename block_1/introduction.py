...
# Importiert Funktionen random.function_name aufgerufen werden können
import random
import time
...
for number in range(-16, 16):
    # Ausführungen hierzu im Skript, siehe Gl. 1.1
    print(f'{number} in byteform {number:#010b}')
...
# Lässt sich nicht als Gleitkommazahl darstellen (s. Skript S. 4)
format(0.1, '.20')
...
# Raw Strings
# Zeigt den String 'as is' -> nützlich für regex Operationen
print(r'Hello\tWorld')

# Formated Strings
some_number = 13
# f'{}' Variablen in geschweifter Klammer
print(f'We can insert variables: {some_number}')
...
# ... wodurch abermals nach einem key gefragt werden kann
nested_dict['Milan']['age']

# Bestehende Einträge können einfach überschrieben werden
nested_dict['Milan']['age'] = 28
...
nested_dict.clear()         # Löscht alle Einträge -> leeres dict
print(nested_dict)
...
# 4 If Condition
# Gibt eine ganze Zahl zwischen 1 und 100 zurück
...
# Gibt den Docstring und den Funktionsheader zurück
help(sum_of_first_n_numbers)
...
def sum_of_numbers(start, end): return sum(range(start, end+1))
...
# * ist notwendig, args Konvention, *numbs funktioniert ebenso
...
def fac(n):
    '''Einfache Funktion zur Berechnung der Fakultät
    einer Zahl'''
    number = 1
...
def merge_sort(array):
    ...
    # ... und rufe Mergesort auf, bis der Basisfall erreicht ist.
...
def merge(left, right):
    ...
    while left_index < len(left) and right_index < len(right):
        if left[left_index] < right[right_index]:
            return_list.append(left[left_index])
            left_index += 1
        else:
...
time_m_sort = time.time() - s_0

print(f'Bubblesort takes {time_b_sort}\nMergesort takes {time_m_sort}\nfor {len_array} values')
...
class Student:
    ...
    def calc_avg(self):
        '''Methode zur Berechnung der durchschnittlichen Noten'''
        return sum(self.grades.values())/len(self.grades)

    def __add__(self, other):
        '''Methode, die die '+' Operationen zwischen zwei Studenten
        über ihren gemittelten Notenschnitt definiert.'''
